...
export default createReducer(initialState, {
  [increment]: (state, action) => {
    state.arrProduct.map((item) => (item.Id === action.payload.Id ? (item.quantity += 1) : item.quantity));
  },
  ...
  [removeProduct]: (state, action) => {
    ...
    state.totapPrise = state.totapPrise - action.payload.quantity * action.payload.price;
  },
