...
const DeliveryInfo = ({ prise, setMakingPurchase }) => {
   ...
   return (
      ...
      <Formik
         ...
         validationSchema={checkedDelivery === "offices" ? offices :
            checkedDelivery === "delivery" ? delivery :
               checkedDelivery === "pickup" ? pickup : null}

         onSubmit={(values, { setSubmitting, resetForm }) => {
            setMakingPurchase('Payment');

            dispatch(postCastumerPhone(values.phone));
            dispatch(postCastumerEmail(values.email));
            dispatch(postCastumerCountry(values.adress));
            dispatch(postCastumerCity(values.city));
            dispatch(postCastumerStreet(values.street));
            dispatch(postCastumerHouse(values.house));
            dispatch(postCastumerApartment(values.apartment));
            dispatch(postCastumerPostcod(values.post));
            dispatch(postCastumerStoryAdress(values.city));
            dispatch(postCastumerDeliveryMethod(checkedDelivery));       
         }}
      >
                  ...
                  <div className="contenerDeliveryInput">
                     ...
                     {(checkedDelivery === "pickup") && <>
                        ...
                        <div className='contenerInput'>
                           ...
                           {touched.adress && errors.adress && <span className="error">{errors.adress}</span>}
                           { /*////////////////////////////////////////////////////////////////*/}
                           <div className='NotVisible'> {values.adress = currentCountryInput}
                           ...
                           <datalist id="city-list">
                              {
                                 currentCity.map(({ city, _id }) => (
                                    <option key={_id} value={city}>{city}</option>
                                 ))}
